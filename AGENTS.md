...
catches the exact class of error you are most likely to make.
...
and with which modules are licensed and installed. If a user is on a different version,
or asks about an entity that is not in `data/`, the authoritative answer comes from
their own server through the `Metadata.*` entities:
