...
| The retention windows behind the `Detail` / `Hourly` / `Daily` rollups | Configured per installation and not in the schema | Compare `SELECT MIN(d.RecordTime) FROM Orion.IpSla.OperationResultsDetail d` against the same query on the `Hourly` and `Daily` entities |
